...
How to run unit tests:
	...
	- Run this command 'jasmine-node spec'
	- Make sure there are no failures

How to setup connection to database:
	- dbuser and dbpassword is generated by me for my database on cloud (server currently uses Ryan's database)
	- get MongoLab (only needed if you want to see the database on their website)
		- once set up, click add account, the URI is:
			- mongodb://<dbuser>:<dbpassword>@ds033390.mongolab.com:33390/stuffly
			- dbuser is your firstname, dbpassword is your lastname
	- edit ./config.json
		- this is JSON, so only change value, not key
		- the new value is your URI, which is :
			- mongodb://<dbuser>:<dbpassword>@ds033390.mongolab.com:33390/stuffly

How to setup push access to AWS:
	- install elasticbeanstalk on your terminal (can be done using NPM)
	- go to root of our server folder
	- type: eb init
		- system will ask you for your pubic/secret key...sent on Facebook
	- ... instructions included later...the rest is annoying/tedious
...
DNE:
	...
	- DO NOT EVER overflow MongoLab database (500MB storage...very unlikely to overflow) ... if close to limit, contact me ASAP
	- DO NOT EVER push to AWS or Git without testing and validating locally ... there is a "blame" on Git
...
Trivial:
	...
	- In HTTPS:
		...
		- Login authentication is kinda complicated ... generally Token based (Facebook) but we can do simple value checking
		- JWT is a new feature for more secure HTTP requests ... out of project scope but look it up for knowledge!!!
